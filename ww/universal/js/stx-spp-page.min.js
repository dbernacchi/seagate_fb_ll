...
$(document).ready(function(){
	...
	if ($("#spp-search-support")[0]) {
		$("#spp-search-support").submit(function(e) {
			var qv=document.getElementById('spp-support-search-textbox').value;
			e.preventDefault();
			if ($.trim(qv) == '' || 
				$.trim(qv) == document.getElementById('defaultText').value) {
				return false;
			}
			...
			if($("#isMgmtStage").val()=="true"){
				window.location.href = "//" + serverHost + localeURL + "/search/?tab=nav-tabs-sp&keyword=" + encoding(qv);
			}else{
				window.location.href = "//" + serverHost + localeURL + "/search/?tab=nav-tabs-sp&keyword=" + qv;
			}
			return false;
		});
	}
