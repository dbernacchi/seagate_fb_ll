...
jQuery.fn.ProductConfigurator = function(method) {
	...
	var isExternal = ProductInfoStruct.releaseList[0].external;
	...
	function createHtml() {
		...
		notifyHtml += "<div id=\"NotifyMe\" class=\"modal hide fade in\">\n";
		notifyHtml += "	<div class=\"modal-body\">\n";
		notifyHtml += "		<button data-dismiss=\"modal\" class=\"nav-footer-close close\">×</button> \n";
		notifyHtml += "		<div data-dismiss=\"modal\" class=\"nav-footer-close \">\n";
		notifyHtml += "			<div class=\"nav-footer-lt\">\n";
		notifyHtml += "			</div>\n";
		notifyHtml += "			<button class=\"nav-footer-close\">Back</button> \n";
		notifyHtml += "		</div>\n";
		notifyHtml += "		<iframe id=\"DrNotifyForm\" src=\"#\"></iframe>\n";
		notifyHtml += "	</div>\n";
		notifyHtml += "</div>\n";
				...
				success: function(data) {  
				    ...
				    if(typeof(store_product_info.storeProduct) == "undefined" || typeof(store_product_info.storeProduct[0]) == "undefined" || store_product_info.storeProduct[0].pid == ""){
						...
						store_response += '     <div class="pdp-buy-seagate">\n';
               	        ...
               	        if (stock_message != labels.instock){
               	            $('#DrNotifyForm').attr('src', 'http://gc.digitalriver.com/store/sgateus/DisplayNotifyMeFormPage/productID.' + PID + '?locale=' + dr_locale);
               	        	store_response += '	                <div class="pdp-get-alert"><a href="#NotifyMe" data-toggle="modal">' + labels.get_alert + '</a></div>\n';
               	        }
	...
	function buildResellersDataVar(country_code) {
		...
		if(country_code != wtb_data_country_code){
			...
			$.ajax({
				...
				url: '/ww/jsp/common/ajax.jsp?country=' + country_code + '&retail=' + isExternal + '&sfdc=1',
				success: function(data) {  
...
$(document).ready(function () {
   ...
   setStayInformedCountry(); // setting the default country in the form based on the user ip geolocation
	...
	function mapProductRowHash(hash){
		var $nav =$(".gn-container");
		if($(hash).length > 0){
			$('html, body').animate({scrollTop: $(hash).offset().top - $nav.outerHeight()}, 500);
		}
	}
	window.onload=function(){
		var hash = location.hash;
		mapProductRowHash(hash); 
	};
	
	$(".pdp-anchor-row li.pdp-ar-item > a").on('click touchstart', function(e){
		e.preventDefault();
	...
	window.onhashchange = function() {  
		mapProductRowHash(location.hash);   
