		...
		return this.each(function() {
			...
			if(tipContent && !settings.clickIt){	
				...
				obj.hover(function(el){
					obj.attr({title: ''});						  
					$('body').append("<div class='"+ settings.toolTipClass +"'><div class='bgHolder'><p class='aToolTipContent'>"+ tipContent +"</p></div><span class='arrow'>&nbsp;</span></div>");
					$('.' + settings.toolTipClass).css({
						position: 'absolute',
						display: 'none',
						zIndex: '50000',
						top: (obj.offset().top - $('.' + settings.toolTipClass).outerHeight() - settings.yOffset)+10 + 'px',
						left: (obj.offset().left + obj.outerWidth()/2 + settings.xOffset)-95 + 'px'
					})
