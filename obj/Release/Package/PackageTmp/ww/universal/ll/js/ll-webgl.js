...
// NOTE: This is temp. in the future it should be common to all APIs
...
var logoGetty = {
  index: 0  // HACK: Use 0 as instagram, but since this uses a different class it's ok
  , doRotation: 0.0
  , MeshMinScaleX: 0.03
  , MeshMaxScaleX: 2.0
  , MeshMinScaleY: 0.03
...
var kCameraDistance = 200.0 * 1.15;
...
if (Environment.isIe() && Environment.isIe() <= 9) {
  ...
  // if(rcLocaleJSDirectory === 'facebook' && renderer != null )
  // {
...
function PrepareDataFacebook() {
  ...
  Params[5].texture = staticData[ "facebook" ];
...
function PrepareDataGetty(api5ID) {
  ...
  Params[5].texture = staticData[ "getty" ];
...
function PrepareFacebookTagline()
{
  ...
  mack.vars.currentApiId = 5;

  //   apiID = 3;
  // GETTY taglines
  copyText = copyTextArray[5];
...
//        GeneratePathOffsetTextures( cubeArrayDimX, cubeArrayDimY, 4, 1.0 );
  ...
  camera = new THREE.PerspectiveCamera(effectController.fieldOfView, window.innerWidth / window.innerHeight, 1.0, 1000.0);
//        camera = new THREE.PerspectiveCamera( effectController.fieldOfView / (window.innerWidth/window.innerHeight), window.innerWidth / window.innerHeight, 1.0, 1000.0 );
  ...
  previousCamPosition = new THREE.Vector3(0, -10, kCameraDistance);
  currentCamPosition = new THREE.Vector3(0, -10, kCameraDistance);
  targetCamPosition = new THREE.Vector3(0, 0, kCameraDistance);
  currentCamLookAt = new THREE.Vector3(0, -10, 0);
...
//        camera.position.set( xOffset, 0, 200 );
  ...
  floorMesh.position.y = -90.0;
  //floorMesh.receiveShadow = true;
  scene.add(floorMesh);



  for (var ai = 0; ai < Params.length - 1; ai++)
  {
    var api = new LL.APIScene();
    api.Init(Params[ai], cubeArrayDimX, cubeArrayDimY, renderer, scene, camera, pathDataTex, effectController, hasLinearFilteringAvailable);
    apiList.push(api);
  }
//        renderApi = apiList[ logoIndex ];
  // Getty
  var api = new LL.APISceneGetty();
  if(rcLocaleJSDirectory == 'facebook')
  {
    Params[5].particleSpeed = 0.1;
  }
  api.Init(Params[5], cubeArrayDimX, cubeArrayDimY, renderer, scene, camera, pathGettyDataTex, effectController, hasLinearFilteringAvailable);
  apiList.push(api);
...
/*    // Facebook API
  ...
  {
      var api = new LL.APISceneGetty();
      api.Init(Params[6], cubeArrayDimX, cubeArrayDimY, renderer, scene, camera, pathGettyDataTex, effectController, hasLinearFilteringAvailable);
      apiList.push(api);
  }*/
...
{
//        effectFXAA.uniforms[ "resolution" ].value.set( 1.0 / window.innerWidth, 1.0 / window.innerHeight );
//        composer.setSize( window.innerWidth * dpr, window.innerHeight * dpr );

//        camera.projectionMatrix.makePerspective( effectController.fieldOfView, window.innerWidth/window.innerHeight, 1, 1000 );
...
//        cameraTime /= kCameraTimeToAnimate;
  ...
  if (!isFullscreen)
  {
    camera.projectionMatrix.makePerspective(effectController.fieldOfView, window.innerWidth / containerHeight, 1, 300.0 );
  }
  else
  {
    camera.projectionMatrix.makePerspective(effectController.fieldOfView, window.innerWidth / window.innerHeight, 1, 300.0 );
  }
...
//        LOG( delta + " - " + currentTime );
  ...
  //   var idx = getLogoIndex(THREE.Math.randInt(0, 5));
...
function FacebookCallFunc_WebGL()
{
  ...
  $.when(LoadTexture(staticData, "getty", 'facebook_img')
          ).done(function()
  {
    
    
    if (Params[5].texture)
      Params[5].texture.dispose();
    Params[5].texture = staticData[ "getty" ];
    SetTexturePointFiltering( Params[5] );
...
function InstagramCallFunc_WebGL()
{
  ...
  {
    if (Params[0].texture)
      Params[0].texture.dispose();
    Params[0].texture = staticData[ "instagram" + apiID + "Json" ];
    SetTextureFiltering(Params[0]);
...
function TwitterCallFunc_WebGL()
{
  ...
  {
    if (Params[1].texture)
      Params[1].texture.dispose();
    // poo('preppzz ')
    poo(staticData[ "twitter" + apiID + "Json" ])
    Params[1].texture = staticData[ "twitter" + apiID + "Json" ];
    SetTextureFiltering(Params[1]);
    /*poo('checking param')*/
    /*poo(Params[1])*/
    SetTextureFiltering(Params[1]);
...
function NYTCallFunc_WebGL()
{
  ...
  {
    if (Params[2].texture)
      Params[2].texture.dispose();
    Params[2].texture = staticData[ "nytimes" + apiID + "Json" ];
    SetTextureFiltering(Params[2]);
...
function AmazonCallFunc_WebGL()
{
  ...
  {
    if (Params[3].texture)
      Params[3].texture.dispose();
    Params[3].texture = staticData[ "amazon" + apiID + "Json" ];
    SetTextureFiltering(Params[3]);
...
function LastFMCallFunc_WebGL()
{
  ...
  {
    if (Params[4].texture)
      Params[4].texture.dispose();
    Params[4].texture = staticData[ "lastfm" + apiID + "Json" ];
    SetTextureFiltering(Params[4]);
